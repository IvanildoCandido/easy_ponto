...
export default function LoginPage() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        // Limpar dados corrompidos do localStorage antes de verificar
        try {
          const authData = localStorage.getItem('easy-ponto-auth');
          if (authData) {
            try {
              JSON.parse(authData);
            } catch (e) {
              // Se não for JSON válido, limpar
              console.warn('Limpando localStorage corrompido');
